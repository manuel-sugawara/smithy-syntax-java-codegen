...
public class SynthesizeServicePluginProvider implements PluginProvider {
    public static final Identifier ID = Identifier.of(SynthesizeServicePluginProvider.class);

    public SynthesizeServicePluginProvider() {
    }

    public static BaseModuleConfig newBaseConfig(String serviceId) {
        if (serviceId == null) {
            throw new IllegalArgumentException("cannot configure plugin, correct serviceId configuration value missing");
        }
        return BaseModuleConfig
            .builder()
            .addEarlyTransformer(DefaultTransformModelTask.builder()
                                                          .taskId(Identifier.of(SynthesizeServiceTransform.class))
                                                          .transform(model -> SynthesizeServiceTransform.transform(model,
                                                                                                                   serviceId))
                                                          .build())
            .build();
    }
