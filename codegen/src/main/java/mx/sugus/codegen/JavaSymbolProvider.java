...
public class JavaSymbolProvider implements SymbolProvider, ShapeVisitor<Symbol> {
    ...
    private JavaSymbolProvider(Model model, JavaCodegenSettings settings) {
        ...
        this.escaper = new ReservedWordsBuilder()
            .loadWords(Objects.requireNonNull(JavaSymbolProvider.class.getResource("java-reserved-words.txt")),
                       keyword -> StringUtils.uncapitalize(serviceName) + keyword)
            .loadWords(Objects.requireNonNull(JavaSymbolProvider.class.getResource("java-system-type-names.txt")),
    ...
    public static JavaSymbolProvider create(CreateSymbolProviderDirective<JavaCodegenSettings> directive) {
        return new JavaSymbolProvider(directive.model(), directive.settings());
    }
