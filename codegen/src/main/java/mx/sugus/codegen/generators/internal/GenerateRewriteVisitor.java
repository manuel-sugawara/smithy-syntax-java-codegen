...
public class GenerateRewriteVisitor {
    ...
    MethodSpec.Builder visitForStructure(ServiceGenerator state, StructureShape shape) {
    ...
    private void addCollectionOfSyntaxNode(ServiceGenerator state, MemberShape member, MethodSpec.Builder builder) {
        var symbolProvider = state.symbolProvider();
    ...
    private Shape memberInnerType(ServiceGenerator state, MemberShape member) {
    ...
    private boolean isCollectionOfSyntaxNode(Set<ShapeId> shapeIds, ServiceGenerator state, MemberShape member) {
        var targetId = member.getTarget();
        var target = state.model().expectShape(targetId);
        if (target.isListShape()) {
            var listShape = target.asListShape().get();
    ...
    void addSingleSyntaxNode(ServiceGenerator state, MemberShape member, MethodSpec.Builder builder) {
        var symbolProvider = state.symbolProvider();
        var memberName = symbolProvider.toMemberJavaName(member);
        var memberType = symbolProvider.toTypeName(member);
        var memberCamelName = memberName.asCamelCase();
    ...
    boolean hasMemberNodes(ServiceGenerator state, Set<ShapeId> shapeIds, StructureShape shape) {
        for (var member : shape.members()) {
            if (shapeIds.contains(member.getTarget())) {
                return true;
            }
